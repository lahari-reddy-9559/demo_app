...
Notes:
- This app supports:
  - Uploading a CSV with a 'text' column and optional 'sentiment' column.
  - Pasting/raw text to get extractive + (optional) abstractive summaries.
  - Topic modeling using LDA (scikit-learn) on the uploaded text.
  - TF-IDF vectorization.
  - Sentiment classifier training (SGD, RandomForest, simple NN) if labels provided.
  - Visualizations: bar charts, stacked sentiment per topic, word clouds.
- Abstractive summarization requires transformers & torch; the app will fall back to extractive-only if unavailable.
"""
...
# Summarization support functions (lightweight extractive + optional abstractive)
import re
import heapq
...
def make_abstractive_pipeline(model_name: str = "t5-small"):
    if not _TRANSFORMERS_AVAILABLE:
        raise RuntimeError("transformers/torch not installed. Install via requirements.txt to enable abstractive summarization.")
...
st.markdown("Upload text data (CSV with 'text' column) or paste text. The app does topic modeling, summarization, sentiment modeling and visualizations.")

# Sidebar options
st.sidebar.header("Options")
uploaded_file = st.sidebar.file_uploader("Upload CSV with a 'text' column (optional)", type=["csv"])
use_sample = st.sidebar.checkbox("Use small sample dataset (demo)", value=True)
...
run_topic = st.sidebar.checkbox("Run topic modeling & visualizations", value=True)
run_sentiment_models = st.sidebar.checkbox("Train sentiment classifiers (if sentiment labels exist)", value=False)

# Load data
def load_demo_df(n=200):
    # Small built-in demo: some sentences covering topics
    texts = [
    ...
    # Add try/except to optionally include a sentiment column for demo
...
if len(df) == 0:
    st.error("No rows to process after cleaning.")
    st.stop()
...
# Sentiment integration & models (if sentiment column present)
if "sentiment" in df.columns and run_sentiment_models:
    st.header("Sentiment - Modeling & Topic Integration")
    ...
    try:
        ...
        y = label_enc.fit_transform(df["sentiment"].fillna("neutral").astype(str))
    ...
    # SGD (logistic)
    st.subheader("SGD (Logistic) per-epoch log loss (quick)")
    ...
    # RandomForest (quick)
    st.subheader("RandomForest - evaluation (increasing trees)")
    ...
    # Simple Neural Network (if TF available)
    if _TF_AVAILABLE:
        st.subheader("Simple Neural Network (TensorFlow)")
        model = Sequential([
        ...
        st.info("TensorFlow not available - skipping NN training. Install tensorflow in requirements to enable.")

    # Integrate topic sentiment summary if topics exist
    if "topic" in df.columns:
        ...
        # Ensure positive/neutral/negative ordering if present
        st.write(stack_df)
...
input_mode = st.radio("Input mode", ["Text input", "Use one document from uploaded dataset"])
text_input = ""
selected_doc_idx = None
if input_mode == "Text input":
    text_input = st.text_area("Enter text to summarize", height=200)
else:
    sel_idx = st.number_input("Select document index (0-based)", min_value=0, max_value=max(0, len(df)-1), value=0)
    selected_doc_idx = int(sel_idx)
    text_input = df.loc[selected_doc_idx, "text"] if len(df)>0 else ""

st.markdown("Summarization options")
ratio = st.slider("Extractive ratio (fraction of sentences kept)", min_value=0.1, max_value=1.0, value=0.3)
...
if st.button("Generate summary"):
    if not text_input or not text_input.strip():
        ...
        with st.spinner("Generating summaries..."):
            extractive = extractive_reduce(text_input, ratio=ratio)
            st.subheader("Extractive Summary")
            st.write(extractive)
            if do_abstractive:
                if not _TRANSFORMERS_AVAILABLE:
                    st.error("Transformers not installed; abstractive summarization unavailable. Install transformers, torch and sentencepiece.")
                else:
                    try:
                        abstractive = abstractive_summarize_text(text_input, model_name=ab_model, max_length=max_len, min_length=20, use_extractive_reduced=True)
...
if "topic" in df.columns:
    st.subheader("Export topic + sentiment summary")
    try:
        # Build aggregates similar to the backend script
        df_int = df.copy()
...
st.markdown("---")
st.caption("Colors and visuals chosen to be light and color-blind friendly (where possible).")
